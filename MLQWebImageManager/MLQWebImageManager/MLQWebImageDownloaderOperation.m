//
//  MLQWebImageDownloaderOperation.m
//  MLQWebImageManager
//
...
@interface MLQWebImageDownloaderOperation ()
...
@implementation MLQWebImageDownloaderOperation
        ...
        if ([self callbacksForKey:kCompletedCallbackKey].count > 0) {
            ...
            if (imageData) {
                ...
                if (self.options & SDWebImageDownloaderIgnoreCachedResponse && [self.cachedData isEqualToData:imageData]) {
                    ...
                    // call completion block with no error
                    [self callCompletionBlocksWithImageData:imageData error:nil finished:YES];
                    [self done];
                }
            } else {
...
- (void)callCompletionBlocksWithError:(nullable NSError *)error {
    [self callCompletionBlocksWithImageData:nil error:error finished:YES];
}

- (void)callCompletionBlocksWithImageData:(nullable NSData *)imageData
                                    error:(nullable NSError *)error
                                 finished:(BOOL)finished {
    NSArray<id> *completionBlocks = [self callbacksForKey:kCompletedCallbackKey];
    dispatch_main_async_safe(^{
        for (SDWebImageDownloaderCompletedBlock completedBlock in completionBlocks) {
            completedBlock(nil, imageData, error, finished);
        }
    });
}
